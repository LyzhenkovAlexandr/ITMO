...
class ExampleTest {
    ...
    fun myTestServices() {
        val config = AirlineConfig(
            audioAlertsInterval = 1.seconds,
            displayUpdateInterval = 100.milliseconds,
            ticketSaleEndTime = 5.minutes,
        )
        val plane1 = Plane("A312", setOf("1A", "1B", "2A", "2B"))
        val plane2 = Plane("B100", setOf())
        val plane3 = Plane("C666", setOf("1A", "1B", "1C", "2A", "2B", "2C"))
        val plane4 = Plane("D777", setOf("1A", "1B"))
        val flightId1 = "111"
        val flightId2 = "222"
        val flightId3 = "111"
        val flightId4 = "123"
        val now = Clock.System.now()
        val flightTime1 = now + 1.hours
        val flightTime2 = now + 2.hours
        val flightTime3 = now + 1.hours
        val flightTime4 = now + 3.hours
        val airlineApplication = AirlineApplication(config, emailService)
        testAndCancel {
            launch { airlineApplication.run() }
            sleep()
            ...
            val booking = airlineApplication.bookingService
            val management = airlineApplication.managementService
            val bufferedEmail = airlineApplication.bufferedEmailService

            management.scheduleFlight(flightId1, flightTime1, plane1)
            sleep(100.milliseconds)

            val freeFlights1 = booking.flightSchedule
            Assertions.assertEquals(1, freeFlights1.size)
            val flight1 = FlightInfo(flightId1, flightTime1, false, flightTime1, null, null, plane1)
            Assertions.assertTrue(flight1 in freeFlights1)

            management.scheduleFlight(flightId2, flightTime2, plane2)
            sleep()

            val freeFlights2 = booking.flightSchedule
            Assertions.assertEquals(1, freeFlights2.size)
            Assertions.assertTrue(flight1 in freeFlights1)

            management.scheduleFlight(flightId3, flightTime3, plane3)
            sleep()

            val freeFlights3 = booking.flightSchedule
            Assertions.assertEquals(1, freeFlights3.size)
            Assertions.assertTrue(flight1 in freeFlights1)

            management.scheduleFlight(flightId4, flightTime4, plane4)
            sleep()

            val freeFlights4 = booking.flightSchedule
            Assertions.assertEquals(2, freeFlights4.size)
            val flight4 = FlightInfo(flightId4, flightTime4, false, flightTime4, null, null, plane4)
            Assertions.assertTrue(flight1 in freeFlights4)
            Assertions.assertTrue(flight4 in freeFlights4)

            booking.buyTicket(flightId1, flightTime1, "1A", "1", "Alexander Lyzhenkov", "myEmail@yandex.ru")
            sleep()
            withTimeout(100.milliseconds) {
                val freeSeats1 = booking.freeSeats(flightId1, flightTime1)
                Assertions.assertEquals(setOf("1B", "2A", "2B"), freeSeats1)
                checkEmails("myEmail@yandex.ru", "successfully")
            }

            booking.buyTicket(flightId2, flightTime2, "1A", "2", "Dmitri Lapin", "hisEmail@gmail.com")
            sleep()
            withTimeout(100.milliseconds) {
                val freeSeats2 = booking.freeSeats(flightId2, flightTime2)
                Assertions.assertEquals(setOf<String>(), freeSeats2)
                checkEmails("hisEmail@gmail.com", "Failed")
            }

            bufferedEmail.send("itmoEmail@gmail.com", "Hello")
            bufferedEmail.send("spbEmail@gmail.com", "my")
            bufferedEmail.send("kotlinEmail@yandex.ru", "friend")
            sleep()
            checkEmails("itmoEmail@gmail.com", "Hello")
            checkEmails("spbEmail@gmail.com", "my")
            checkEmails("kotlinEmail@yandex.ru", "friend")

            management.setGateNumber(flightId4, flightTime4, "25")
            sleep()

            val flight6 = FlightInfo(flightId4, flightTime4, false, flightTime4, null, "25", plane4)
            Assertions.assertTrue(flight6 in booking.flightSchedule)

            management.setCheckInNumber(flightId4, flightTime4, "10")
            sleep()

            val flight7 = FlightInfo(flightId4, flightTime4, false, flightTime4, "10", "25", plane4)
            Assertions.assertTrue(flight7 in booking.flightSchedule)
        }
    }
    ...
    fun myTestDisplay() {
        ...
        testAndCancel {
            ...
            val management = airlineApplication.managementService
            val display = airlineApplication.airportInformationDisplay(this)
            Assertions.assertTrue(display.value.departing.isEmpty())

            management.scheduleFlight(flightId1, flightTime1, plane1)
            sleep()

            management.delayFlight(flightId1, flightTime1, flightTime1 + 1.hours)
            sleep(100.milliseconds)

            Assertions.assertEquals(1, display.value.departing.size)
            Assertions.assertEquals(flightTime1 + 1.hours, display.value.departing[0].actualDepartureTime)
            ...
            Assertions.assertEquals(3, display.value.departing.size)
            Assertions.assertTrue(display.value.departing.any { it.flightId == flightId1 })
            Assertions.assertTrue(display.value.departing.any { it.flightId == flightId2 })
            Assertions.assertTrue(display.value.departing.any { it.flightId == flightId4 })
        }
    }
    ...
    companion object {
        ...
        fun finish() {
            ...
            if (elapsedTime > timeoutMillis) {
                throw TimeoutException("Total test execution time exceeded 30 seconds")
            }
        }
    }
}
